...
def get_shop_bot_token():
    try:
        response = requests.get('http://127.0.0.1:5000/api/latest/shop_bot')
        if response.status_code == 200:
            return response.json().get('token')
        else:
            print('Failed to fetch Shop Bot token:', response.status_code)
    except Exception as e:
        print('Error:', e)
    return None

def main():
    shop_bot_token = get_shop_bot_token()
    ...
    if shop_bot_token:
        ...
        print('Failed to fetch Shop Bot token. Make sure the endpoint is working correctly.')
